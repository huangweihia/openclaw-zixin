...
@section('content')
    <div class="max-w-3xl mx-auto">
        ...
        @if (($canReadFull ?? true) && $sop->summary)
        ...
        @if ($sop->contact_note)
            <div class="oc-surface p-6 mb-8">
                ...
                @else
                    <p class="text-sm oc-muted m-0">已开启会员可见，请 <a href="{{ route('pricing') }}#plan-vip" class="oc-link font-semibold" style="text-decoration: none;">开通 VIP</a> 后查看。</p>
                @endif
            </div>
        @endif
        @else
        ...
        <section class="oc-surface p-6">
            ...
            @if (! ($canReadFull ?? true))
                <p class="text-sm oc-muted m-0">开通 VIP 后可查看全文、清单与评论。</p>
            @elseif ($canComment)
                @auth
                ...
                @endauth
            @else
                <p class="text-sm mb-4 oc-muted m-0">本 SOP 已限制为 <strong>VIP</strong> 可评论，请 <a href="{{ route('pricing') }}#plan-vip" class="oc-link font-semibold" style="text-decoration: none;">开通会员</a>。</p>
            @endif
