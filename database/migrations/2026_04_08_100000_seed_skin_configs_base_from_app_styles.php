...
/**
 * 与 resources/js/frontend/App.vue 中 :root[data-skin="style1"]～style7 七套预设一致，
 * 作为换肤基础数据；code 使用 style1～style7，便于与前台 data-skin 对齐。
 */
return new class extends Migration
{
    public function up(): void
    {
        ...
        $now = now();
        foreach ($this->baseThemes() as $row) {
            $code = $row['code'];
            ...
            $exists = DB::table('skin_configs')->where('code', $code)->exists();
            if ($exists) {
                unset($payload['created_at']);
                DB::table('skin_configs')->where('code', $code)->update($payload);
            } else {
                $payload['created_at'] = $now;
                DB::table('skin_configs')->insert($payload);
            }
        }
    }
    ...
    public function down(): void
    {
        ...
        DB::table('skin_configs')->whereIn('code', [
    ...
    private function baseThemes(): array
    {
        return [
            ...
            [
                'name' => '暗夜黑',
                'code' => 'style5',
                'description' => '深色界面，高对比点缀，对应 App.vue style5；演示环境可作为 VIP 示例皮肤',
                'preview_image' => null,
                'type' => 'vip',
                'sort' => 5,
                'is_active' => true,
