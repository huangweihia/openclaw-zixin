...
class SideHustleCaseCommentController extends Controller
{
    public function store(Request $request, SideHustleCase $sideHustleCase): RedirectResponse|JsonResponse
    {
        ...
        if (! empty($data['parent_id'])) {
            ...
            if (
                $parent->commentable_type !== $sideHustleCase->getMorphClass()
                || (int) $parent->commentable_id !== (int) $sideHustleCase->id
            ) {
                return $this->wantsCommentJson($request)
                    ? response()->json(['error' => '回复目标无效'], 422)
    ...
    public function reply(Request $request, Comment $comment): RedirectResponse|JsonResponse
    {
        ...
        if (! $case instanceof SideHustleCase) {
            abort(404);
        }

        $root = $comment->threadRoot();
        $reply = $case->comments()->create([
            'user_id' => $request->user()->id,
            'parent_id' => $root->id,
            'content' => $data['content'],
        ]);
    ...
    public function like(Request $request, Comment $comment): JsonResponse
    {
        ...
        if (! $comment->commentable instanceof SideHustleCase) {
            abort(404);
        }
    ...
    {
        return $request->wantsJson() || $request->ajax() || $request->boolean('ajax');
    }
}
